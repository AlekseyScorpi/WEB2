...
export default function Home() {
  ...
  return (
    <Flex  flexDirection={window.innerHeight > window.innerWidth ? 'column-reverse' : 'row'} justifyContent={['center', 'center', 'center', 'space-between', 'space-between', 'space-between']} h='100%'>
      <Box as={motion.div} initial={{x : -50, opacity : 0}} animate={{x : 0, opacity : 1}} transition='0.7s' fontSize={[20, 24, 28, 32, 32, 32]} w='100%' display='flex' justifyContent='center' alignItems='center' maxH={['100%', '100%', '100%', '100%', '100%', '100%']}>
